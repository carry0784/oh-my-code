            ...
            if len(self.schedule.history) >= self.schedule.stagnation_window:
                ...
                if stag_improvement < self.schedule.plateau_threshold:
                    ...
                    self.schedule.current_rate = min(
                        self.schedule.current_rate * 2.0,
                        self.schedule.max_rate
                    )
                    logger.info("mutation_stagnation_spike", rate=round(self.schedule.current_rate, 4), gen=generation)
                    return self.schedule
            
            # Regular plateau — increase moderately
            self.schedule.current_rate = min(
                self.schedule.current_rate * 1.3,
                self.schedule.max_rate
            )
            logger.info("mutation_plateau_increase", rate=round(self.schedule.current_rate, 4), gen=generation)
        else:
            # Improving — decrease rate (exploit)
            self.schedule.current_rate = max(
                self.schedule.current_rate * 0.9,
                self.schedule.min_rate
            )
            logger.info("mutation_improving_decrease", rate=round(self.schedule.current_rate, 4), gen=generation)
