...
def run_governance_monitor() -> GovernanceMonitorReport:
    ...
    summary = (
        f"G-MON: {ok_count} OK, {warn_count} WARN, {fail_count} FAIL — overall {overall.value}"
    )
    ...
    try:
        ...
        if pattern_count >= _PATTERN_FAIL_COUNT:
            return Indicator(
                ...
                f"<{_PATTERN_FAIL_COUNT}",
                "Multiple recurring failure patterns",
            )
        if pattern_count >= _PATTERN_WARN_COUNT:
            return Indicator(
                ...
                IndicatorStatus.WARN,
                f"{pattern_count} patterns",
                f"<{_PATTERN_WARN_COUNT}",
                "Recurring failure detected",
            )
        return Indicator(
            "PATTERN_CHECK", IndicatorStatus.OK, "0 patterns", f"<{_PATTERN_WARN_COUNT}"
        )
    except Exception as e:
    ...
    try:
        ...
        if forbidden_blocks > 0:
            return Indicator(
                "FORBIDDEN_CHECK",
                IndicatorStatus.FAIL,
                f"{forbidden_blocks} blocks",
                "0",
                "Forbidden action attempts detected — investigate immediately",
            )
...
def _check_token_usage() -> Indicator:
    ...
    try:
        ...
        if token_budget is None:
            return Indicator(
                "TOKEN_USAGE", IndicatorStatus.OK, "no_budget", "n/a", "LLM_TOKENS budget not set"
            )
        ...
        if ratio >= _TOKEN_FAIL_RATIO:
            return Indicator(
                "TOKEN_USAGE",
                IndicatorStatus.FAIL,
                f"{current}/{limit} ({ratio:.0%})",
                f"<{_TOKEN_FAIL_RATIO:.0%}",
    ...
    try:
        ...
        if rate < _SUCCESS_FAIL:
            return Indicator(
                "AGENT_SUCCESS",
                IndicatorStatus.FAIL,
                f"{rate:.0%} ({success}/{total})",
                f">{_SUCCESS_FAIL:.0%}",
                "Agent success rate critically low",
            )
