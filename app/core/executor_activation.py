...
def evaluate_activation(
    ...
    items.append(ActivationCheckItem(
        name="approval_exists", passed=apr_ok,
        observed=str(apr_ok), expected="true", rule_refs=["Art43"],
    ))
    if not apr_ok:
    ...
    items.append(ActivationCheckItem(
        ...
        expected="MATCH", rule_refs=["Art33", "Art39"],
    ))
    if not pol_ok:
        reasons.append(ActivationReason.POLICY_NOT_MATCH if policy else ActivationReason.MISSING_POLICY_SNAPSHOT)
    ...
    items.append(ActivationCheckItem(
        name="gate_open", passed=gate_ok,
        observed=gate_obs, expected="OPEN", rule_refs=["Art43"],
    ))
    if not gate_ok:
    ...
    items.append(ActivationCheckItem(
        name="preflight_ready", passed=pf_ok,
        observed=pf_obs, expected="READY", rule_refs=["Art43"],
    ))
    if not pf_ok:
    ...
    items.append(ActivationCheckItem(
        name="ops_score_ok", passed=sc_ok,
        observed=sc_obs, expected=f">={ops_score_threshold}", rule_refs=["Art41"],
    ))
    if not sc_ok:
    ...
    try:
        ...
        scope_ok = False
    items.append(ActivationCheckItem(
        name="scope_compatible", passed=scope_ok,
        observed=f"{execution_scope}⊆{apr_scope}", expected="compatible",
    ...
    items.append(ActivationCheckItem(
        name="no_critical_alert", passed=crit_ok,
    ...
    items.append(ActivationCheckItem(
        name="activation_approval_exists", passed=act_apr_ok,
        observed=str(act_apr_ok), expected="true", rule_refs=["Art43"],
    ))
    if not act_apr_ok:
    ...
    expected_hash = compute_intent_hash(
        policy_snap, execution_scope, target_symbol, requested_action, timestamp_bucket,
    )
    # For now, computed hash is the reference (no prior hash to compare against in dev)
    hash_ok = len(expected_hash) == 64
    items.append(ActivationCheckItem(
        name="intent_hash_valid", passed=hash_ok,
        observed=expected_hash[:16] + "...", expected="valid SHA-256",
        rule_refs=["Art43"],
    ))
    if not hash_ok:
    ...
    return ActivationReceipt(
        ...
        activation_approval_id=activation_approval.activation_approval_id if activation_approval else None,
...
def _collect_approval() -> dict | None:
    try:
        ...
        return {
            ...
            "scope": r.approval_scope.value if hasattr(r.approval_scope, "value") else str(r.approval_scope),
...
def _recheck_ops_score(threshold: float) -> tuple[bool, str]:
    try:
        ...
        s = _compute_ops_score(
            IntegrityPanel(exchange_db_consistency="unknown", snapshot_age_seconds=None,
                           position_mismatch=False, open_orders_mismatch=False,
                           balance_mismatch=False, stale_data=False),
...
def _recheck_critical(since: str) -> tuple[bool, str]:
    try:
        ...
        cnt = sum(1 for e in fl.list_entries(limit=50) if e.get("policy_urgent") and e.get("executed_at", "") > since)
...
def _store_activation_evidence(
    activation_id: str, decision: ActivationDecision,
