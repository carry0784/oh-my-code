"""
Kiwoom Securities (키움증권) Exchange Adapter
I-02: 키움증권 어댑터 추가

키움증권은 CCXT 미지원이므로 키움 Open API+ (REST) 기반 자체 구현.
국내 주식 현물 전용:
...
# Kiwoom Open API+ REST base URLs
_KIWOOM_BASE_REAL = "https://openapi.koreainvestment.com:9443"
_KIWOOM_BASE_DEMO = "https://openapivts.koreainvestment.com:29443"
...
class KiwoomExchange(BaseExchange):
    ...
    def __init__(self):
        super().__init__(settings.kiwoom_app_key, settings.kiwoom_app_secret)
        self._base_url = _KIWOOM_BASE_DEMO if settings.kiwoom_demo else _KIWOOM_BASE_REAL
        self._account_no = settings.kiwoom_account_no
        self._account_suffix = settings.kiwoom_account_suffix
    ...
    async def _ensure_token(self) -> str:
        ...
        try:
            ...
            logger.info("Kiwoom token acquired")
            return self._token
        except Exception as e:
            logger.error("Kiwoom token acquisition failed", error=str(e))
            raise
    ...
    async def create_order(
        ...
        try:
            ...
            resp.raise_for_status()
            result = resp.json()
            logger.info("Kiwoom order created", symbol=symbol, side=side)
            return result
        except Exception as e:
            logger.error("Kiwoom order failed", symbol=symbol, error=str(e))
            raise

    async def cancel_order(self, order_id: str, symbol: str) -> dict[str, Any]:
        """주문 취소."""
        token = await self._ensure_token()
        tr_id = "VTTC0803U" if settings.kiwoom_demo else "TTTC0803U"
        resp = await self._client.post(
    ...
    async def fetch_order(self, order_id: str, symbol: str) -> dict[str, Any]:
        """주문 조회 — 일별 주문 내역으로 조회."""
        token = await self._ensure_token()
        tr_id = "VTTC8001R" if settings.kiwoom_demo else "TTTC8001R"
    ...
    async def fetch_positions(self) -> list[dict[str, Any]]:
        ...
        """
        token = await self._ensure_token()
        tr_id = "VTTC8434R" if settings.kiwoom_demo else "TTTC8434R"
        try:
            resp = await self._client.get(
            ...
            logger.error("Kiwoom fetch_positions failed", error=str(e))
            raise
