...
class MetricComparison(BaseModel):
    ...
    metric_name: str = ""
    current_window: WindowSnapshot = Field(
        default_factory=lambda: WindowSnapshot(window_label="current")
    )
    previous_window: WindowSnapshot = Field(
        default_factory=lambda: WindowSnapshot(window_label="previous")
    )
...
class TrendObservationSchema(BaseModel):
    ...
    blocked_trend: MetricComparison = Field(
        default_factory=lambda: MetricComparison(metric_name="blocked_total")
    )
    pending_retry_trend: MetricComparison = Field(
        default_factory=lambda: MetricComparison(metric_name="pending_retry_total")
    )
    review_trend: MetricComparison = Field(
        default_factory=lambda: MetricComparison(metric_name="review_total")
    )
    watch_trend: MetricComparison = Field(
        default_factory=lambda: MetricComparison(metric_name="watch_total")
    )
