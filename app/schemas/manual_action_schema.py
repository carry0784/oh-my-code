...
class ManualActionChainState(BaseModel):
    ...
    def all_pass(self) -> bool:
        ...
        stages = [
            self.pipeline, self.preflight, self.gate,
            self.approval, self.policy, self.risk,
            self.auth, self.scope, self.evidence,
        ]
    ...
    def first_blocked_stage(self) -> Optional[str]:
        ...
        for name in [
            "pipeline", "preflight", "gate", "approval",
            "policy", "risk", "auth", "scope", "evidence",
        ]:
