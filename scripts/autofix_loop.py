...
def _get_autofix_decision(failure_type: str, recurrence: str) -> tuple[str, str]:
    ...
    """
    policy = AUTOFIX_POLICY.get(
        failure_type, {"FIRST": "MANUAL", "REPEAT": "DENY", "PATTERN": "DENY"}
    )
...
def _record_failure_pattern(test_id: str, detail: str, iteration: int, scope: str) -> dict:
...
def _calculate_success_rate(patterns: list[dict], failure_type: str) -> float:
    ...
    matching = [
        p for p in patterns if p.get("failure_type") == failure_type and p.get("fix_attempted")
    ]
    if not matching:
        return 0.0
...
def _suggest_fix_strategy(patterns: list[dict], failure_type: str, recurrence: str) -> str:
...
def _update_pattern_fix_result(
    patterns: list[dict], test_id: str, iteration: int, succeeded: bool, strategy: str
) -> None:
...
def analyze_failures(
    test_data: dict, patterns: list[dict], iteration: int, scope: str
) -> list[dict]:
...
def autofix_loop(scope: str, max_iter: int = MAX_ITERATIONS, dry_run: bool = False) -> dict:
    """
    ...
    for i in range(1, max_iter + 1):
        ...
        if dry_run:
            ...
            if iter_result.get("analysis"):
                ...
                manual_items = [
                    a for a in current_analysis if a.get("autofix_decision") == "MANUAL"
                ]
                ...
                if allow_items:
                    ...
                    for a in allow_items[:MAX_FILES_PER_FIX]:
                        if a.get("suggested_strategy"):
                            print(
                                f"         Strategy: {a['suggested_strategy']} (success rate: {a.get('success_rate', 0):.0%})"
                            )
...
def main():
    ...
    parser.add_argument(
        "--scope",
        default="governance",
        choices=["governance", "constitution", "dashboard", "monitor", "agents", "all"],
    )
    parser.add_argument(
        "--max-iter",
        type=int,
    ...
    parser.add_argument("--dry-run", action="store_true", help="Analyze only, no autofix")
