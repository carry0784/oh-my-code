...
**별칭: CG-2B Exercisability Recovery Package**

Date: 2026-04-01
Status: **APPROVED (CONDITIONAL GO)** — A 검수 통과 2026-04-01
...
### 사전 점검 (shadow 시작 전) — 구조 준비도 점검

> **주의**: Day 0는 "후보가 생기는 날만 shadow 시작" 하는 선택 편향 게이트가 아님.
> 구조적으로 코드가 올바르게 연결되어 있는지 확인하는 exercisability preflight임.

| # | 항목 | 확인 방법 | 합격 기준 |
|---|------|-----------|-----------|
| PRE-1 | RSI 전략 클래스 로드 가능 | `from strategies.rsi_strategy import RSICrossStrategy` 성공 | import 오류 없음 |
| PRE-2 | genome→strategy 분기 정상 | `_genome_to_strategy(genome)` RSI/SMA 양쪽 반환 확인 | 두 경로 모두 BaseStrategy 인스턴스 |
| PRE-3 | 진화 루프 완주 가능 | `AdvancedStrategyRunner.run_evolution()` 에러 없이 완료 | status=OK |
| PRE-4 | 변경 파라미터 적용 확인 | n_islands=5, pop=10, gen=10, lookback=30, OHLCV=500 | 설정값 일치 |
| PRE-5 | 전체 테스트 통과 | `pytest tests/ -x` | 0 failures |
| PRE-6 | 금지 범위 미침범 | dry_run=True, GovernanceGate 불변, execution path 불변 | grep/AST 확인 |

> **Day 0 비선별 원칙**: Day 0 preflight는 live 성공 선별 gate가 아니며,
> Day 0의 결과(registry, governance 등)는 재Shadow 7일 evidence의 일부로 계산하지 않는다.
> Day 0는 오직 구조 준비도(readiness)와 경로 실행 가능성(exercisability)만 확인한다.
...
| 7 | `docs/operations/evidence/cr045_shadow_redesign_package.md` | EDIT | Day 0 preflight 구조 준비도 전환, 문서 정합성 |

**총 7파일 (코드 4, 테스트 2, 문서 1)**
...
Alias: CG-2B Exercisability Recovery Package
