...
| Layer | 이름 | 거버넌스 |
|-------|------|----------|
| Work State Machine | DOC-L2-EXT-API-GOV §2 정의 상태 + TransitionGuards | B2 |

소스: work_state.py, 상태 분류 SSOT: DOC-L2-EXT-API-GOV §2

## 3. 구현 과제

1. WorkStateEnum 22개 상태 존재 확인 (분류 기준: DOC-L2-EXT-API-GOV §2)
2. 정상 경로 전이 순서 검증 (경로 정의: DOC-L2-EXT-API-GOV §2)
3. 실패 복구 경로 전이 순서 검증 (경로 정의: DOC-L2-EXT-API-GOV §2)
4. ValidatingCheck 10개 순서 검증 (체크리스트 정의: DOC-L2-EXT-API-GOV §3)
...
- 10-check 순서(DOC-L2-EXT-API-GOV §3) 및 실패 처리 확인
