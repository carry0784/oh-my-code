...
### 5.5 Reference Check (운영 문서 변경 시)

MERGE_RULES.md 또는 pull_request_template.md를 변경하는 PR에서는
아래 정합성 체크를 수행한다.

| 체크 항목 | 확인 내용 |
|-----------|-----------|
| MB/MA 조건 수 | MERGE_RULES.md의 조건 수와 OPS_GUIDE.md §2 참조 관계도의 범위가 일치하는가 |
| 템플릿 섹션 | pull_request_template.md의 섹션 구성과 OPS_GUIDE.md §2 관계도가 일치하는가 |
| 용어 통일 | 동일 개념에 다른 용어가 사용되지 않는가 |
| SSOT 원칙 | OPS_GUIDE.md가 정의를 복제하지 않고 참조만 유지하는가 |

불일치 발견 시: 같은 PR에서 정정하거나, 범위 초과면 별도 ops PR로 분리한다.
