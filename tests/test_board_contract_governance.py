...
class TestBoardDictFree:
    ...
    def test_no_dict_fields_in_board_response(self):
        ...
        for name, field_info in FourTierBoardResponse.model_fields.items():
            ...
            # Direct dict type
            assert annotation is not dict, f"Field '{name}' is dict — must be typed schema (OC-09)"
    ...
    def test_cleanup_action_summary_is_typed(self):
        ...
        assert (
            FourTierBoardResponse.model_fields["cleanup_action_summary"].annotation
            is CleanupActionSummary
        )
...
class TestSchemaGovernance:
    ...
    def test_board_response_field_names_snapshot(self):
        ...
        expected = {
            ...
            "order_tier",
            "derived_flags",
            "top_block_reasons_all",
            "recent_lineage",
            "cross_tier_orphan_count",
            "cross_tier_orphan_detail",
            "cleanup_candidate_count",
            "cleanup_action_summary",
            "cleanup_simulation_only",
            "observation_summary",
            "decision_summary",
            "decision_card",
            "review_volume",
