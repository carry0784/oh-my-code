...
class TestC30QueryFilter:
    ...
    def test_count_total(self):
        store = RetryPlanStore()
        store.enqueue(channel="ext", reason="a", reliability_tier="t", retryable=True, incident="1")
        store.enqueue(
            channel="slack", reason="b", reliability_tier="t", retryable=True, incident="2"
        )
    ...
    def test_pending_count(self):
        store = RetryPlanStore()
        r1 = store.enqueue(
            channel="ext", reason="a", reliability_tier="t", retryable=True, incident="1"
        )
        store.enqueue(
            channel="slack", reason="b", reliability_tier="t", retryable=True, incident="2"
        )
...
class TestC30CapacityEviction:
    ...
    def test_eviction_preserves_newest(self):
        ...
        store.enqueue(
            channel="old", reason="a", reliability_tier="t", retryable=True, incident="inc_old"
        )
        store.enqueue(
            channel="mid", reason="a", reliability_tier="t", retryable=True, incident="inc_mid"
        )
        store.enqueue(
            channel="new", reason="a", reliability_tier="t", retryable=True, incident="inc_new"
        )
        pending = store.list_plans(status="pending")
