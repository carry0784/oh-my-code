...
class TestC14ModuleStructure:
    ...
    def test_returns_dict(self):
        result = route_snapshot({"highest_incident": "NONE", "active_incidents": [], "degraded_reasons": []})
...
class TestC14Forbidden:
    ...
    def test_no_notification_sending(self):
        ...
        content = ALERT_ROUTER_PATH.read_text(encoding="utf-8")
        sending_terms = ['requests.post', 'httpx', 'smtp', 'discord', 'slack', 'webhook', 'send_message']
        for term in sending_terms:
            assert term not in content, f"Sending term '{term}' found — router should only decide routing"
