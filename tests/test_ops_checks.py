...
class TestFailClosed:
    ...
    def test_daily_returns_fail_on_import_error(self, monkeypatch):
        ...
        original = (
            mod.run_daily_ops_check.__wrapped__
            if hasattr(mod.run_daily_ops_check, "__wrapped__")
            else None
        )
...
class TestBeatSchedule:
    ...
    def test_beat_has_ops_daily(self):
        ...
        assert (
            schedule["ops-daily-check"]["task"] == "workers.tasks.check_tasks.run_daily_ops_check"
        )
    ...
    def test_beat_has_ops_hourly(self):
        ...
        assert (
            schedule["ops-hourly-check"]["task"] == "workers.tasks.check_tasks.run_hourly_ops_check"
        )
    ...
    def test_total_beat_entries(self):
        ...
        schedule = celery_app.conf.beat_schedule
        assert len(schedule) == 15  # 15 entries: 13 base + sol-paper-trading-hourly + ppf-shadow-eval-hourly
