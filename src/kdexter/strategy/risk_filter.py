...
class RiskFilter:
    ...
    def check(self, signal: Signal, account: AccountState) -> RiskCheckResult:
        ...
        if signal.is_expired:
            signal.mark_expired()
            return self._fail(signal, checks_run, checks_passed, "Signal expired (TTL exceeded)")
        checks_passed += 1
        ...
        if signal.symbol in self._limits.forbidden_symbols:
            ...
            return self._fail(
                signal, checks_run, checks_passed, f"Forbidden symbol: {signal.symbol}"
            )
        checks_passed += 1
        ...
        if signal.exchange in self._limits.forbidden_exchanges:
            ...
            return self._fail(
                signal, checks_run, checks_passed, f"Forbidden exchange: {signal.exchange}"
            )
        checks_passed += 1
    ...
    def _fail(
        self, signal: Signal, checks_run: int, checks_passed: int, reason: str
    ) -> RiskCheckResult:
        result = RiskCheckResult(
            passed=False,
            signal_id=signal.signal_id,
